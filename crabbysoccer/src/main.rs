...
impl std::fmt::Display for ApplicationType {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        let utype = if let ApplicationType::Server = &self {
            ("Server", ApplicationType::Server as u8)
        } else {
            ("Client", ApplicationType::Server as u8)
        };
...
fn parse_args(args: std::env::Args) -> (ApplicationType, Option<bool>) {
    ...
    match args.get(1) {
        Some(s) => match s.to_lowercase() {
            ...
            _ => {
                println!("Invalid ApplicationType argument provided, assuming Client...");
